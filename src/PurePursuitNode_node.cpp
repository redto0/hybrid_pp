...
void PurePursuitNode::ackerman_cb(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  transformed_goal_pose =
    tf_buffer_->transform(msg_to_goal_pose, rear_axle_frame_, transform_tolerance_);

  // Calc look ahead distance
  look_ahead_distance_ =
    std::clamp(k_dd_ * speed_, min_look_ahead_distance_, max_look_ahead_distance_);
  ...
  alpha_ = std::atan2(
    transformed_goal_pose.pose.pose.position.y - transformed_robot_pose.pose.pose.position.y,
...
void PurePursuitNode::odom_speed_cb(const ackermann_msgs::msg::AckermannDrive::SharedPtr msg)
{
  speed_ = msg->speed;
}
