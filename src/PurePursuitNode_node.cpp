...
void PurePursuitNode::ackerman_cb(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    set_speed = 0.6 * std::sqrt(gravity_constant * std::abs(distance_to_icr));
    ...
    if (steering_angle < 0) {
        // Loop runs until the point generated h its the look ahead distance raduis
        for (double index = 0; distance(path_graph_point, zero) - look_ahead_distance <= 0; index += 0.001) {
            ...
            path_graph_point.x = distance_to_icr * -std::sin(index);
            path_graph_point.y = distance_to_icr * -std::cos(index) + distance_to_icr;
        ...
        for (double index = -3.14 / 2; distance(path_graph_point, zero) - look_ahead_distance <= 0; index += 0.001) {
            // Sets the x and y coords using ploar coodiants the the ICR as the origin
            path_graph_point.x = distance_to_icr * std::cos(index);
            path_graph_point.y = distance_to_icr * std::sin(index) + distance_to_icr;
    ...
    for (double index = 0; index <= 2 * 3.14; index += 0.01) {
        look_ahead_graph_point.x = look_ahead_distance * std::cos(index);
        look_ahead_graph_point.y = look_ahead_distance * std::sin(index);
